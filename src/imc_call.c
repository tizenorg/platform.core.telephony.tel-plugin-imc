...
static void on_response_set_sound_path(TcorePending *p, int data_len,
					const void *data, void *user_data)
{
	const TcoreAtResponse *at_resp = data;
	ImcRespCbData *resp_cb_data = user_data;
	TelCallResult result = TEL_CALL_RESULT_FAILURE;
	TelCallSoundPath *snd_path;
	CoreObject *co_call = tcore_pending_ref_core_object(p);

	/* Notify audio control plugin about sound path */
	snd_path = (TelCallSoundPath *)resp_cb_data->data;
	tcore_object_send_notification(co_call, TCORE_NOTIFICATION_CALL_SOUND_PATH,
				sizeof(TelCallSoundPath), snd_path);
	...
	TcoreObjectResponseCallback cb, void *cb_data)
{
	...
	if (g_str_has_prefix(cp_name, "imcmodem")) {
		...
		/* Response callback data */
		resp_cb_data = imc_create_resp_cb_data(cb, cb_data,
					&sound_path_info->path, sizeof(TelCallSoundPath));
		...
		tcore_at_prepare_and_send_request(co, "AT+XDRV=40,6,0,4",
				NULL, TCORE_AT_COMMAND_TYPE_NO_RESULT,
				TCORE_PENDING_PRIORITY_DEFAULT, NULL, NULL,
				NULL, NULL, NULL,
				(guint)0, NULL, NULL);
		...
		ret = tcore_at_prepare_and_send_request(co, "AT+XDRV=40,6,0,2",
				"+XDRV", TCORE_AT_COMMAND_TYPE_SINGLELINE,
				TCORE_PENDING_PRIORITY_DEFAULT, NULL,
				on_response_set_sound_path, resp_cb_data,
				NULL, NULL, (guint)0, NULL, NULL);
